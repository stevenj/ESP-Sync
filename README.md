...
| CHK  | 0x0000-0xFFFF | FLETCHER-16 checkusm of all data from STX to FO3 |
| DATA | 0x00-0xFF x N | Variable Length data body. |
| CHK2 | 0x0000-0xFFFF<br>0x00000000-0xFFFFFFFF | FLETCHER-16 for Data payloads <= 64KB<br>ADLER-32 for Data Payloads > 64KB |

ALL Multibyte fields are sent MOST SIGNIFICANT BYTE FIRST.

### STX - Start Message Sentinal
...
| 0x74 | Received   | Response to the File command [SIZE] |
...
### CHK - Header Checksum
This is the Flether-16 Checksum of all bytes from STX through to the end of SIZ/OPT.  This checksum was chosen because it is almost as good as a CRC-16 but uses far less program and data space, and is very fast.
...
The Data payload check is either 2 or 4 bytes long.  

For a Data payload that is <= 65536 bytes long then the CHK2 is 2 bytes and is a FLETCHER-16 checksum of all bytes in the Data payload ONLY.

For a Data payload that is > 65536 bytes long then the CHK2 is 4 bytes and is a ADLER-32 checksum of all bytes in the Data payload ONLY.
...
| CHK2   | 2   | Fletcher-16 Checksum of DAY thru SECOND |
...
| CHK2  | 2    | Fletcher-16 Checksum of SIZE |
...
| CHK2  | 2    | Fletcher-16 Checksum of OPT |
...
| CHK2  | 2/4    | Fletcher-16 OR Adler-32 Checksum of all Data |
...
| CHK2  | 2    | Fletcher-16 Checksum of NAME |
...
| SIZE  | 4    | Size of the SPIFFS |
| FREE  | 4    | Free space in the SPIFFS |
| CHK2  | 2    | Fletcher-16 Checksum of SIZE & FREE |

### 0x64 - File - Send a File to the Slave
